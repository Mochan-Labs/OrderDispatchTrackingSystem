...
router.get('/api/products', ensureAdminOrOffice, async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      FROM odts.product_master p
      LEFT JOIN odts.user_master u ON u.user_id = p.updated_by
...
router.post('/api/products', ensureAdminOrOffice, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO odts.product_master (product_name, product_desc, product_is_active_flag, created_by, updated_by, created_at, updated_at)
...
router.put('/api/products/:id', ensureAdminOrOffice, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE odts.product_master SET product_name=$1, product_desc=$2, product_is_active_flag=$3, updated_by=$4, updated_at=now()
